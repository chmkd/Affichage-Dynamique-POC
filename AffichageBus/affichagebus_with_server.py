...
class AffichageBusWithServer:
    ...
    def display_video(self, filepath, duration):
        """Affiche une vidéo avec décodage matériel accéléré"""
        try:
            # Ouvrir avec accélération matérielle si disponible
            ...
            cap.set(cv2.CAP_PROP_BUFFERSIZE, 3)  # Buffer minimal pour réduire latence

            # Détecter le FPS réel
            fps = cap.get(cv2.CAP_PROP_FPS)
            if not fps or fps <= 0 or fps > 1000:
                fps = 30.0  # Augmenté à 30 par défaut
            ...
            # Boucle de lecture ULTRA-RAPIDE
            frame_count = 0
            while True:
                # Vérifier durée écoulée (utiliser time.time() au lieu de pygame.time)
                ...
                # Redimensionner directement à la taille écran (OpenCV ultra-rapide)
                frame = cv2.resize(frame, (self.screen_width, self.screen_height), interpolation=cv2.INTER_NEAREST)
                
                # Conversion BGR->RGB
                frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

                # Création surface optimisée
                ...
                # TIMING BASÉ SUR LE TEMPS RÉEL (pas de clock.tick qui limite)
                frame_count += 1
                ...
                # Si sleep_time < 0, on est en retard, on continue sans pause
