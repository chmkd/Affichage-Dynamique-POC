...
MEDIA_DURATION = 15             # secondes pour chaque image/vidéo
...
def page_media_image(path):
    ...
    try:
        ...
        img = pygame.transform.smoothscale(img, (LEFT_W, HEIGHT))
...
def page_media_video(path):
    """Lit une vidéo mp4 et l'affiche pendant MEDIA_DURATION secondes"""
    ...
    while time.time() - start_time < MEDIA_DURATION:
        ...
        if not ret:
            break
        # Adapter la vidéo à la taille de la partie gauche
        frame = cv2.resize(frame, (LEFT_W, HEIGHT))
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frame_surface = pygame.surfarray.make_surface(frame.swapaxes(0,1))
        screen.blit(frame_surface, (0, 0))

        # On dessine aussi le panneau droit
        draw_right_panel()
        pygame.display.flip()

        # Permettre la sortie propre
...
def main():
    ...
    while True:
        ...
        # Rotation de page après le temps prévu
